...
for step in range(epochs+1):
    ...
    if step%200 == 0: 
        ...
        plt.plot(steps, y.flatten().cpu(), 'r-')                         # 训练集
        plt.plot(steps, prediction.data.cpu().numpy().flatten(), 'b-')   # 训练集结果

        steps1 = torch.linspace(4 * math.pi, 16 * math.pi, time_step)     # 测试集
        x1 = steps1
        x1 = torch.unsqueeze(x1, 1)
        x1 = torch.unsqueeze(x1, 0)
        x1 = x1.to(device)
        prediction1, hiden_state1 = rnn(x1, hiden_state)
        plt.plot(steps1, prediction1.data.cpu().numpy().flatten(), 'g-')
        #plt.show()
        plt.draw()
        plt.pause(0.8)
        plt.close()
